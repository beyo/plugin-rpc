...
function connectionUrl() {
  ...
  url = url + (url.indexOf('?') === -1 ? '?' : '&') + 'id=' + encodeURIComponent(config.owner);
...
function emitMessage(name, data, target) {
  ...
  enveloppe = buildEnveloppe(message, Enveloppe.TYPE_REQUEST);
  // FIXME: change this when jQuery 3.0 is out
  def = $.Deferred();
...
function onMessage(name, callback) {
  if (typeof name === 'string') {
    ...
    common.scanObject('', name, registerHandler);
  } else {
...
function removeListener(name) {
  if (typeof name === 'string') {
    ...
    common.scanObject('', name, unregisterHandler);
  } else {
    throw new Error('Invalid message handler arguments');
  }
}
