...
function bindSpark(rpc, spark) {
  ...
  spark.on('end', function onEnd() {
    Object.keys(rpc._pendingRequests).forEach(function (enveloppeId) {
      var pending = rpc._pendingRequests[enveloppeId];

      if (pending.sparkId === spark.id) {
        pending.callback({
          error: 'Disconnected'
        });
      }
...
function messageRouter(rpc, message) {
  ...
  if (!message.target || message.target.indexOf('server') !== -1) {
    promises.push(rpc.events.emit(message.action, message).then(function (results) {
  ...
  rpc.clients.forEach(function (client) {
    if ((message.owner !== client.id) && (!message.target || message.target.indexOf(String(client.id)) !== -1)) {
